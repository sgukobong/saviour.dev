...
const FloatingChat: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const initialY = window.innerHeight - 620;
  ...
  if (!isOpen) {
      return (
          <button 
            onClick={toggleChat}
            className="fixed bottom-10 right-10 z-[60] w-16 h-16 bg-black text-white rounded-full shadow-2xl flex items-center justify-center hover:scale-110 transition-all duration-300"
  ...
  return (
    <div 
      ...
      className="fixed z-[60] w-[90vw] md:w-[400px] h-[600px] flex flex-col bg-white border border-black/10 rounded-3xl shadow-[0_20px_60px_rgba(0,0,0,0.15)] overflow-hidden animate-fade-in"
    >
      <div 
        onMouseDown={handleMouseDown}
        className="bg-slate-50 border-b border-black/5 p-5 flex justify-between items-center cursor-move select-none"
      >
         <div className="flex items-center gap-3">
            <div className={`w-3 h-3 rounded-full transition-all duration-500 ${isSpeaking ? 'bg-black animate-pulse' : 'bg-black'}`}></div>
            <div className="flex flex-col">
                <span className="font-bold text-xs uppercase tracking-widest text-black">Studio Assistant</span>
                {isSpeaking && <span className="text-[10px] text-black font-black animate-pulse">SPEAKING</span>}
            </div>
         </div>
         <div className="flex items-center gap-2">
             <button 
                onClick={toggleChat}
                className="text-slate-900 hover:text-black transition-colors p-2"
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-6 custom-scrollbar bg-white">
         {messages.map((msg, idx) => (
            <div key={idx} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
               <div className={`max-w-[85%] p-4 text-sm font-semibold rounded-2xl leading-relaxed ${
                   ...
                   : 'bg-slate-100 text-black border border-black/5 rounded-tl-sm shadow-sm'
               }`}>
                  <Markdown
                    components={{
                        a: ({ node, ...props }) => {
                            ...
                            return (
                                <a
                                    {...props}
                                    className="text-black underline font-black hover:text-slate-600 transition-colors cursor-pointer"
                                    onClick={(e) => {
               ...
               {msg.role === 'model' && hasBookingLink(msg.text) && (
                 <a 
                   ...
                   className="mt-3 inline-flex items-center gap-2 bg-black text-white px-5 py-2.5 rounded-full text-xs font-black hover:bg-slate-800 transition-colors shadow-lg"
                 >
                   Book a Slot ↗
                 </a>
               )}
         ...
         {isTyping && (
             <div className="flex justify-start">
                 <div className="bg-slate-100 border border-black/5 p-4 rounded-2xl rounded-tl-sm flex gap-1.5">
      ...
      <div className="p-5 bg-slate-50 border-t border-black/10">
        <form onSubmit={handleSubmit} className="relative flex gap-3 items-center">
            <button 
                ...
                className={`p-3.5 rounded-full transition-all duration-300 ${
                    ...
                    : 'bg-white border border-black/10 text-slate-800 hover:border-black'
                }`}
            >
                {isListening ? (
            ...
            <div className="relative flex-grow">
                <input 
                    ...
                    className="w-full bg-white border-2 border-black/10 rounded-full pl-5 pr-14 py-3.5 text-sm text-black font-bold focus:outline-none focus:border-black transition-all placeholder:text-slate-400"
                />
                <button 
                    type="submit"
                    disabled={!input.trim() || isTyping}
                    className="absolute right-2 top-1/2 -translate-y-1/2 p-2.5 bg-black text-white rounded-full hover:bg-slate-800 transition-colors disabled:opacity-0"
                >
